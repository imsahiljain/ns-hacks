...
router.post('/checkloguser', async (req, res) => {
    ...
    const checklogsfirst = await logs.findOne({email: email, password: password});
    console.log(checklogsfirst);
        if(!checklogsfirst){
            res.send( JSON.stringify({ url: "", toaststatus:"error" ,additional: "Incorrect credentials !" }));
            return;
        }
        else{
            ...
            //redirection
            switch(tempUserType){
                case "admin": return res.send(JSON.stringify({url: "/admin",toaststatus:"success", additional: "Login Successful"}));
                case "student": return res.send(JSON.stringify({url: "/student/classes",toaststatus:"success", additional: "Login Successful"}));
                case "teacher": return res.send(JSON.stringify({url: "/teacher/classes",toaststatus:"success", additional: "Login Successful"}));
              }
        }
      });
