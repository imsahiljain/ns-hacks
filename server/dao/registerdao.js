...
router.post('/checkreguser', async (req, res) => {
    ...
    if (firstname == "" ||lastname == ""|| email == ""|| password == "" || type == ""){
        res.send( JSON.stringify({ url: "", toaststatus:"info", additional: "Please fill all info!" }));
        return;
    }
    else {
    ...
    let checklogsfirst = await logs.exists({email: email});
        if(checklogsfirst){
            res.send( JSON.stringify({ url: "",toaststatus:"info", additional: "User already exists !" }));
            return;
        }
        else{
            ...
            let platform = new logs({ usertype: type, firstname: firstname, lastname:lastname, email:email, password: password});
            platform = await platform.save();
            //redirecting
            switch(type){
                case "admin": return res.send(JSON.stringify({url: "/admin", toaststatus:"success", additional: "Registration Successful"}));
                case "student": return res.send(JSON.stringify({url: "/student", toaststatus:"success", additional: "Registration Successful"}));
                case "teacher": return res.send(JSON.stringify({url: "/teacher/classes", toaststatus:"success", additional: "Registration Successful"}));
            }
        }
    }
});
